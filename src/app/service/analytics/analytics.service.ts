...
export class AnalyticsService {
 private apiUrl = 'http://localhost:8080/hastarandevu/analytics';
  ...
  getAppointmentCountByClinic(): Observable<any[]> {
    return this.http.get<any[]>(`${this.apiUrl}/appointments/clinic`, { 
      headers: this.getHeaders() });
  }

  getAppointmentCountByDate(): Observable<any[]> {
    return this.http.get<any[]>(`${this.apiUrl}/appointments/date`, { 
      headers: this.getHeaders() });
  }

  getAppointmentCountByStatus(): Observable<any[]> {
    return this.http.get<any[]>(`${this.apiUrl}/appointments/status`, { 
      headers: this.getHeaders() });
  }
  ...
  getComplaintCountByStatus(): Observable<any[]> {
    return this.http.get<any[]>(`${this.apiUrl}/complaints/status`, { 
      headers: this.getHeaders() });
  }

  getComplaintCountByClinic(): Observable<any[]> {
    return this.http.get<any[]>(`${this.apiUrl}/complaints/clinic`, { 
      headers: this.getHeaders() });
  }

  getAppointmentCountByTimeSlot(): Observable<any[]> {
    return this.http.get<any[]>(`${this.apiUrl}/appointments/time-slot`, {
       headers: this.getHeaders() });
  }
...
getUserCountByGender(): Observable<any[]> {
  return this.http.get<any[]>(`${this.apiUrl}/users/genders`, { 
    headers: this.getHeaders() });
}

// Kan grubu dağılımı (ARH_POS, ORH_NEG vs.)
getUserCountByBloodType(): Observable<any[]> {
  return this.http.get<any[]>(`${this.apiUrl}/users/blood-types`, {
     headers: this.getHeaders() });
}
